...
const youtrack = {
    ...
    getIssueFieldById(issueId, fieldId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { data: prField } = yield this.youtrackAxiosInstance.get(`/api/issues/${issueId}/customFields/${fieldId}`, {
                    params: {
...
function deletePrLinkInPrField(prField, prLink) {
    const oldPrFieldValue = prField.value;
    if (!oldPrFieldValue)
...
function updatePrLinkInPrField(prField, prLink, merged) {
    const oldPrFieldValue = prField.value;
    if (!oldPrFieldValue)
        ...
        try {
            ...
            switch (github.context.payload.action) {
                case "opened": {
                    ...
                    const prField = yield youtrackApi.getIssueFieldById(taskNum, youtrackFieldId);
